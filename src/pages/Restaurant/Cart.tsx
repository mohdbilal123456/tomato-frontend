...
function Cart() {
  ...
  const increaseQty = async (itemId: string) => {
    ...
    try {
      const data = await cartAPI.increementQuantity(itemId)
      await fetchCart()
  ...
  const decreaseQty = async (itemId: string) => {
    ...
    try {
      const data = await cartAPI.decreementQuantity(itemId)
      await fetchCart()
      toast.success(data?.message)
    } catch (error) {
      console.log("error", error)
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 space-y-6">
        ...
        <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
          ...
          <button
            ...
            className="mt-3 w-full rounded-lg bg-[#232222] cursor-pointer py-3 text-sm font-semibold text-white hover:bg-gray-900 flex justify-center items-center gap-3"
